...
class GetVaultItemsUseCase(private val repository: SecureVaultRepository) {
...
class LockFileInVaultUseCase(private val repository: SecureVaultRepository) {
    suspend operator fun invoke(
        sourceFile: File, category: String = "Other", notes: String = "",
        deleteOriginal: Boolean = true, isDecoy: Boolean = false
    ): Result<VaultItem> = repository.lockFileInVault(sourceFile, category, notes, deleteOriginal, isDecoy)
}

class RestoreVaultItemUseCase(private val repository: SecureVaultRepository) {
    suspend operator fun invoke(vaultItemId: String, destinationDir: File? = null): Result<File> =
        repository.restoreFileFromVault(vaultItemId, destinationDir)
}

class ExportVaultItemUseCase(private val repository: SecureVaultRepository) {
    suspend operator fun invoke(vaultItemId: String, destinationFile: File): Result<File> =
        repository.exportFileFromVault(vaultItemId, destinationFile)
}

class DeleteVaultItemUseCase(private val repository: SecureVaultRepository) {
    suspend operator fun invoke(vaultItemId: String): Result<Boolean> =
        repository.deleteVaultItemPermanently(vaultItemId)
}

class VaultAuthUseCase(private val repository: SecureVaultRepository) {
